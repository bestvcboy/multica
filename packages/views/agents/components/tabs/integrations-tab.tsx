...
import { LweixinAgentBindButton } from "../../../settings/components/lweixin-tab";
import { LweixinMark } from "../../../settings/components/lweixin-mark";
...
export function IntegrationsTab({ agent }: { agent: Agent }) {
  ...
  const canManageLweixin = isWorkspaceAdmin;
  ...
  const lweixinConfigured = lweixinListing?.configured === true;
  const lweixinInstallSupported = lweixinListing?.install_supported === true;
  const lweixinHasActiveInstall =
    lweixinListing?.installations.some(
      (inst) => inst.agent_id === agent.id && inst.status === "active",
    ) ?? false;
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border">
        <div className="flex items-start gap-3 p-4">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border bg-muted/40 text-muted-foreground">
            <LweixinMark className="h-4 w-4" />
          </span>
          <div className="min-w-0 flex-1 space-y-1">
            <h3 className="text-body font-medium">{ts(($) => $.lweixin.section_title)}</h3>
            <p className="text-caption leading-relaxed text-muted-foreground">
              {ts(($) => $.lweixin.page_description)}
            </p>
          </div>
        </div>
        <div className="border-t px-4 py-3">
          {!canManageLweixin ? (
            ...
            <p className="text-caption text-muted-foreground">
              {ts(($) => $.lweixin.not_enabled_title)}
            </p>
          ) : !lweixinInstallSupported && !lweixinHasActiveInstall ? (
            <div className="space-y-1">
              <p className="text-caption font-medium">{ts(($) => $.lweixin.preview_title)}</p>
              <p className="text-caption text-muted-foreground">
                {ts(($) => $.lweixin.preview_description)}
              </p>
            </div>
          ) : (
            <LweixinAgentBindButton agentId={agent.id} agentName={agent.name} />
          )}
        </div>
      </section>
    </div>
