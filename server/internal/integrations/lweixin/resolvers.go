...
// originLweixinChat is the issue.origin_type label for the LWEIXIN /issue
// command; migration 533 widens the CHECK for it.
const originLweixinChat = "lweixin_chat"

// NewLweixinResolverSet assembles the LWEIXIN ResolverSet on the generic
// channel_* queries. No typing notifier: the LWEIXIN server has no chat
// action API to call.
func NewLweixinResolverSet(q *db.Queries, tx engine.TxStarter, replier engine.OutboundReplier) engine.ResolverSet {
	return engine.ResolverSet{
		Installation: &installationResolver{q: q},
		Identity:     &identityResolver{q: q},
		Dedup:        &deduper{q: q},
		Session: &sessionBinder{session: engine.NewChatSession(q, tx, TypeLweixin, engine.SessionTitles{
			Group:    "LWEIXIN group",
			Direct:   "LWEIXIN direct message",
			Fallback: "LWEIXIN chat",
		})},
		Audit:      &auditor{q: q},
		Replier:    replier,
		OriginType: originLweixinChat,
	}
}
...
// lweixinBindingConfig is the opaque outbound routing persisted on the chat
// binding config. The binding key already equals the chat id; the field
// keeps the mapping explicit for the outbound path.
type lweixinBindingConfig struct {
	ChatID string `json:"chat_id"`
}

// lweixinSessionRouting derives the session-isolation key and binding config
// from one inbound message: one continuous session per chat (peer wxid or
// group room id). Pure function.
func lweixinSessionRouting(msg channel.InboundMessage) (string, []byte) {
	cfg, _ := json.Marshal(lweixinBindingConfig{ChatID: msg.Source.ChatID})
	return msg.Source.ChatID, cfg
}

func decodeLweixinRaw(msg channel.InboundMessage) (lweixinRawEvent, error) {
	var raw lweixinRawEvent
	if len(msg.Raw) == 0 {
		return lweixinRawEvent{}, errors.New("lweixin: inbound message Raw is empty")
	}
	if err := json.Unmarshal(msg.Raw, &raw); err != nil {
		return lweixinRawEvent{}, fmt.Errorf("decode lweixin inbound raw: %w", err)
	}
	return raw, nil
}
...
func (r *sessionBinder) EnsureSession(ctx context.Context, p engine.EnsureSessionParams) (pgtype.UUID, error) {
	bindingKey, config := lweixinSessionRouting(p.Message)
...
func (r *sessionBinder) BindMedia(ctx context.Context, p engine.BindMediaParams) (engine.BindMediaResult, error) {
	// v1 carries no inbound media: nothing to bind, placeholder stands.
	_ = p
...
func (r *auditor) RecordDrop(ctx context.Context, instID pgtype.UUID, msg channel.InboundMessage, reason engine.DropReason) error {
	raw, _ := decodeLweixinRaw(msg) // best-effort; a decode miss still audits
