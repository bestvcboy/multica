...
- Deployment target and scope: the production Multica is the fork's own stack on the cysmall ECS (compose project multica2, public entry https://paperclip.cheyishang.com -- YARP routes `/` to web and `/v1/**` plus `/ws*` to backend). Every fork merge lands there before clients pick it up, and every desktop/mobile client must point at that URL, not the upstream cloud. Ops details and credentials: [docs/engineering/cysmall-ecs-ops-reference.md](docs/engineering/cysmall-ecs-ops-reference.md).
